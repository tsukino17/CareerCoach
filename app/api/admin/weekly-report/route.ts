...
export async function sendReport(report: Awaited<ReturnType<typeof buildWeeklyReport>>, title = '网站周报') {
  ...
  const toRaw = process.env.ANALYTICS_REPORT_TO_EMAIL || process.env.ADMIN_EMAILS || '';
  const to = toRaw.split(',').map((item) => item.trim()).filter(Boolean);
  if (!apiKey || !from || to.length === 0) throw new Error('weekly_report_email_not_configured');
  const response = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from, to, subject: `EchoTalent ${title} ${report.period.start} 至 ${report.period.end}`, html: renderWeeklyReportHtml(report, title), text: renderWeeklyReportText(report, title) }),
  });
